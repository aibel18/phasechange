...
int MiniGL::idlefunchz = 60;
int MiniGL::width = 0;
int MiniGL::height = 0;
int MiniGL::widthF = 0;
int MiniGL::heightF = 0;
int MiniGL::desWidth = 310;
int MiniGL::desHeight = 100;
Quaternionr MiniGL::m_rotation;
Real MiniGL::m_zoom = 1;
...
//double iniPos = -0.6;//heat
double iniPos = -1.3;//simulation
//double iniPos = -1.5;//evaporation,condensation
...
void MiniGL::setViewport(float pfovy, float pznear, float pzfar, const Vector3r &peyepoint, const Vector3r &plookat)
{
	...
	m_translation = Vector3r(transformation(0, 3), iniPos, -7);
...
void MiniGL::timedDisplay(int n) {

	MiniGL::display();
	glutTimerFunc(MiniGL::idlefunchz, MiniGL::timedDisplay, 0);
...
void MiniGL::display ()
{
	...
	glutSwapBuffers();
	glReadPixels(desWidth, desHeight, widthF, heightF, GL_RGBA, GL_UNSIGNED_BYTE, pixels);
}

 void MiniGL :: saveFrame(char *prefix,int test_id ,float frame_id, unsigned int color_max, unsigned int pixel_nbytes) {
	 if (!pixels)
		 return;
	size_t i, j, k, cur;
	enum Constants { max_filename = 256 };
	char filename[max_filename];
	snprintf(filename, max_filename, "%s_%d_%f.ppm", prefix,test_id, frame_id);
	FILE *f = fopen(filename, "w");
	fprintf(f, "P3\n%d %d\n%d\n", widthF, heightF, 255);
	for (i = 0; i < heightF; i++) {
		for (j = 0; j < widthF; j++) {
			cur = pixel_nbytes * ((heightF - i - 1) * widthF + j);
...
/*
void MiniGL::saveFrame(char *prefix, int test_id, int frame_id, unsigned int color_max, unsigned int pixel_nbytes) {
	if (!pixels)
		return;
	enum Constants { max_filename = 256 };
	char filename[max_filename];
	snprintf(filename, max_filename, "%s_%d_%d.ppm", prefix, test_id, frame_id);
	...
	fwrite(pixels, sizeof(int)*widthF*heightF, 1, ffmpeg);
...
void MiniGL::init(int argc, char **argv, const int width, const int height, const int posx, const int posy, const char *name)
{
	...
	glutTimerFunc(60, MiniGL::timedDisplay, 0);
#ifndef __APPLE__
	...
	widthF = width - 2*desWidth;
	heightF = height - 2 * desHeight;

	pixels = new GLubyte [4 * widthF * heightF];
}
...
void MiniGL::loadTexture(GLuint &textureID)
{
	/*
	...
	if (imageSize == 0)    imageSize = width*height * 1; // 3 : one byte for each Red, Green and Blue component
	...
	glTexImage2D(GL_TEXTURE_2D, 0, GL_DEPTH_COMPONENT16, width, height, 0, GL_DEPTH_COMPONENT, GL_UNSIGNED_BYTE, data);
