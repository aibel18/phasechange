...
namespace PBD
{
	...
	Real sigmoide(Real val, Real lf, Real begin, Real offset);
	Real sigmoide2(Real val, Real li,Real lf, Real begin, Real offset);
	Real logarithmic(Real val, Real lf, Real begin, Real offset,Real curve);
	Real logarithmic2(Real val, Real lf, Real begin, Real offset, Real curve);
	...
	public:
		...
		newDistanceConstraint()  {
			m_numberOfBodies = 2;
			m_bodies = new unsigned int[2];
			lambda = 0;
		}
		virtual bool evalConstraint(FluidModel &model);
		virtual bool initConstraint(FluidModel &model, const unsigned int particle1, const unsigned int particle2);
		virtual bool solvePositionConstraint(FluidModel &model);
		virtual bool updateConstraint(FluidModel &model);
